...
MERGE INTO menus m USING (SELECT 'Yearly Cash' label,'/report/yearly' link,null icon) AS n ON n.label=m.label WHEN NOT MATCHED THEN INSERT(label,link,icon) VALUES(n.label,n.link,n.icon);
...
MERGE INTO menu_has_menu m USING (SELECT 12 menu_id,10 parent_id) AS n ON n.menu_id=m.menu_id AND n.parent_id=m.parent_id WHEN NOT MATCHED THEN INSERT(menu_id,parent_id) VALUES(n.menu_id,n.parent_id);
...
MERGE INTO role_has_menu r USING (SELECT 1 role_id,12 menu_id) AS n ON n.role_id=r.role_id AND n.menu_id=r.menu_id WHEN NOT MATCHED THEN INSERT(role_id,menu_id) VALUES(n.role_id,n.menu_id);
