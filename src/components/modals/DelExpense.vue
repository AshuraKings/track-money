...
import { delExpenses } from '../../api/master'

const { expense } = defineProps({ expense: Object, })
...
function submit() {
    ...
    delExpenses({ id: expense.id }).then(r => {
...
<template>
    ...
    <BaseModal @onClose="cancelled" :open>
        <p>
            Are you sure for deleting expense <b>{{ expense.nm }}</b> ?
        </p>
