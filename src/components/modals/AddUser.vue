...
const invalidForm = computed(() => router.loading || !nm.value || username.value.length < 5 || pass.value.length < 8 || repass.value !== pass.value)
...
function cancelled() {
    open.value = false
    success.value = error.value = nm.value = username.value = pass.value = repass.value = ''
    roles.value = []
    emit('onClose')
}
