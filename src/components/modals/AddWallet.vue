...
import { addWallet } from '../../api/master'
import MessageModal from '../MessageModal.vue'
import DialInput from '../DialInput.vue'

const emit = defineEmits(['onClose'])
const open = ref(false), success = ref(''), error = ref(''), nm = ref(''), balance = ref(0)
const router = useRouter()

const invalidForm = computed(() => router.loading || !nm.value || balance.value < 0)
...
function submit() {
    ...
    addWallet({ nm: nm.value, balance: balance.value }).then(r => {
...
<template>
    ...
    <BaseModal @onClose="cancelled" :open>
        ...
        <form>
            ...
            <DialInput type="number" id="balance" name="balance" label="Balance" :disabled="router.loading"
                v-model="balance" />
        </form>
